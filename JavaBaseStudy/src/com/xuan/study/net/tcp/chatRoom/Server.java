package com.xuan.study.net.tcp.chatRoom;

import java.io.BufferedInputStream;
import java.io.BufferedOutputStream;
import java.io.DataInputStream;
import java.io.DataOutputStream;
import java.io.IOException;
import java.net.ServerSocket;
import java.net.Socket;
import java.util.ArrayList;
import java.util.List;

import com.xuan.study.io.close.FileUtil;

public class Server {

	// 存放已连接的客户端
	private List<MyChannal> clients = new ArrayList<MyChannal>();

	public static void main(String[] args) throws IOException {
		new Server().start();
	}

	/**
	 * 启动服务器
	 * 
	 * @throws IOException
	 */
	@SuppressWarnings("resource")
	public void start() throws IOException {
		ServerSocket server = new ServerSocket(9999);
		while (true) {
			Socket socket = server.accept();
			MyChannal channal = new MyChannal(socket);
			// 将连接上服务器的客户端加入List
			clients.add(channal);
			new Thread(channal).start();
		}
	}

	/**
	 * 为每一个客户端分配一个线程 设置为内部类便于访问Server类的信息
	 */
	private class MyChannal implements Runnable {

		// 输入流
		private DataInputStream dis;
		// 输出流
		private DataOutputStream dos;
		// 线程是否可运行的标志
		private boolean isRunning = true;

		private String name;

		public MyChannal(Socket socket) {
			try {
				dis = new DataInputStream(new BufferedInputStream(socket.getInputStream()));
				dos = new DataOutputStream(new BufferedOutputStream(socket.getOutputStream()));
				// 用户连接时获取发送的用户名
				this.name = dis.readUTF();
				// 给连接的用户发一条欢迎
				sendMessage("欢迎进入聊天室");
				// 给其他的用户发一条提示
				sendMessageToOthers(name + " 进入聊天室");
			} catch (IOException e) {
				// e.printStackTrace();
				/*
				 * 创建线程出现异常，设置标志为不可运行，关闭流
				 */
				isRunning = false;
				FileUtil.close(dos, dis);
			}
		}

		/**
		 * 接收客户端的数据
		 * 
		 * @return
		 */
		private String receiveMessage() {
			String msg = "";
			try {
				msg = dis.readUTF();
			} catch (IOException e) {
				// e.printStackTrace();
				isRunning = false;
				FileUtil.close(dis);

				// 出现异常或关闭移除自身
				clients.remove(this);
				sendMessageToOthers(name + " 离开了聊天室");
			}
			return msg;
		}

		/**
		 * 向客户端发送数据
		 * 
		 * @param msg
		 */
		private void sendMessage(String msg) {
			if (msg == null || msg == "") {
				return;
			}
			try {
				dos.writeUTF(msg);
				// 写完最好flush()下
				dos.flush();
			} catch (IOException e) {
				// e.printStackTrace();
				isRunning = false;
				FileUtil.close(dos);

				// 出现异常或关闭移除自身
				clients.remove(this);
				sendMessageToOthers(name + " 离开了聊天室");
			}
		}

		/**
		 * 将某个客户端发送的信息，发送给其他客户端
		 */
		private void sendMessageToOthers(String msg) {
			// 查看消息是私聊还是群聊,@开头是私聊
			if (msg.startsWith("@")) {
				// 获取私聊的名字
				String name = msg.substring(1, msg.indexOf(":"));
				// 获取私聊的内容
				String content = msg.substring(msg.indexOf(":") + 1);
				/*
				 * System.out.println(name); System.out.println(content);
				 */
				for (MyChannal client : clients) {
					if (client.name.equals(name)) {
						client.sendMessage(name + "向你私聊:" + content);
					}
				}
			} else {
				// 遍历容器
				for (MyChannal other : clients) {
					if (other == this) {
						continue;
					}
					other.sendMessage(name + "向所有人说：" + msg);
				}
			}
		}

		@Override
		public void run() {
			while (isRunning) {
				String msg = receiveMessage();
				if (msg == null || msg == "") {
					continue;
				}
				sendMessageToOthers(msg);
			}
		}

	}
}
